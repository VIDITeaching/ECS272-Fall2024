...
export default function Example() {
    ...
    const dataFromCSV = async () => {
      try {
        const csvData = await d3.csv('/data/demo.csv', d => {
          ...
          return {category: d.category, total: +d.value};
        });
        setBars(csvData);
      } catch (error) {
  ...
  function initChart() {
    ...
    let yExtents = d3.extent(bars.map((d: CategoricalBar) => d.total as number)) as [number, number]
    ...
    const chartBars = chartContainer.append('g')
      ...
      .attr('y', (d: CategoricalBar) => yScale(d.total) as number)
      // specify the size of the rectangle
      .attr('width', xScale.bandwidth())
      .attr('height', (d: CategoricalBar) => Math.abs(yScale(0) - yScale(d.total))) // this substraction is reversed so the result is non-negative
      .attr('fill', 'teal')
