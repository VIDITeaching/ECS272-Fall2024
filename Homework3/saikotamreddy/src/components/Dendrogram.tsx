...
export default function Dendrogram({selectedYear}) {
    ...
    const dataFromCSV = async () => {
      try {
        ...
        for(let student in csvData){
          ...
          if(selectedYear!=null && selectedYear!="All Years"){
            if(selectedYear.toLowerCase()!=csvData[student].year.toLowerCase()){
              continue
            }
          }
...
svg.append('text')
  ...
  .style('fill', '#333')
  .text('Mental Issue Overlap- ' + yearText); 
...
node.append('circle')
  .attr('r', d => (parseInt(d.data.name.split(" ").reverse()[0])/4))
...
node.append('text')
  .attr('dy', '.35em')
  .attr('x', d => (d.children ? 45 : 10))
  .attr('y', d => (d.data.name.charAt(0)===('S') ? 40 : (d.children ? 20 : 0)))
