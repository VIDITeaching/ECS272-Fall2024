  ...
  methods: {
    drawBarChart() {
      ...
      const processedData = ageGroups.map(group => {
        const groupData = this.data.filter(d => 
          d["Income"] !== "" && 
          !isNaN(+d["Income"]) && 
          !isNaN(+d["Age"]) && 
          +d["Age"] >= group.low && 
        ...
        const averageIncome = groupData.length > 0 
          ? d3.mean(groupData, d => +d["Income"]) 
          : 0;
      ...
      // Y-axis
      svg.append('g')
        .call(d3.axisLeft(y).ticks(10).tickFormat(d => `$${d / 1000}k`))  // Increments of $10,000
        .selectAll("text")
