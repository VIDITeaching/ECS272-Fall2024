...
import dataFromJson from '../../data/demo.json'; /* Example of reading in data directly from file */
...
export default {
    ...
    methods: {
        ...
        async read() {
            let readFromCSV = await d3.csv('../../data/car_prices.csv', (d: d3.DSVRowString<"make" | "sellingprice">) => {
                // This callback allows you to rename the keys, format values, and drop columns you don't need
                return { category: d.make, value: +d.sellingprice } as { category: string, value: number }
                // If you want that typing error to go away, use below
                //return { category: d.category, value: +(d.value ?? 0) } as { category: string, value: number }
            });
            this.bars = readFromCSV
        },
        initChart() {
