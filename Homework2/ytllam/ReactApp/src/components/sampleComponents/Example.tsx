...
export default function Example() {
  ...
  // the underlying data for the bars. not related to bar components themselves.
  const [bars, setBars] = useState<CategoricalBar[]>([]);

  // State for tracking window size. Used to dynamically resize graph to fit browser window...
  ...
  // create ref to html element so resizeObserver can hook into it.
  // Note: updating ref does not trigger rerenders.
  ...
  // Runs after initial render and after all updates to specified dependencies.
  // Empties the bar-svg SVG and re-renders the chart.
  // In this case, this effect only runs if bars or size are changed from last render. This will happen when:
  // - the async finishes populating bars from csvData.
  // - size is updated 
  useEffect(() => {
  ...
  // className is used to specify style used in style.css
  // svg is created and width/height set to fill viewport. d3 code selects this svg for editing by id using d3.select('#bar-svg')
  return (
    <>
