...
function initChart() {
    ...
    let yExtents = d3.extent(bars.map((d) => d.gpa))
    // This is to get the unique categories from the data using Set, then store in an array.
    let xCategories = [...new Set(bars.map((d) => d.depression))]
    ...
    const barEles = chartContainer.append('g')
        ...
        .attr('x', (d) => xScale(d.depression))
        .attr('y', (d) => yScale(d.gpa))
        // specify the size of the rectangle
        .attr('width', xScale.bandwidth())
        .attr('height', (d) => Math.abs(yScale(0) - yScale(d.gpa))) // this substraction is reversed so the result is non-negative
        .attr('fill', 'teal')
