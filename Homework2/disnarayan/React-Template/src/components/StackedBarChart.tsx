...
const StackedBarChart: React.FC = () => {
 ...
 useEffect(() => {
   ...
   svg.selectAll('g.category')
     ...
     .attr('transform', d => {
       const xPos = x0(d[0]) || 0;
       const centerOffset = (x0.bandwidth() - (x1.bandwidth() * Array.from(d[1]).length)) / 2;
       return `translate(${xPos + centerOffset},0)`;
     })
     .selectAll('rect')
     .data(d => d[1])
     .join('rect')
